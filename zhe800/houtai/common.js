...
var checkReg = {
    ...
    url: function (str) {//验证url
        var reg = /^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/;
        return reg.test(str);
    },
    idcar: function (str) {//身份证
    ...
    user: function (str) {//账号用户名
        var reg = /^[a-zA-Z][a-zA-Z0-9_]{6,12}$/;
        return reg.test(str);
    },
    password: function (str) {//密码
        var reg = /^[a-zA-Z]\w{6,18}$|^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}$/;
        return reg.test(str);
    },
    time: function (str) {//时间
